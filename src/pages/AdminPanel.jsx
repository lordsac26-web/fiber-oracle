...
import { ArrowLeft, MessageSquare, FileText, CheckCircle, XCircle, Clock, AlertCircle, User, Calendar, Users, BarChart3, TrendingUp, Activity, UserCheck, UserX, Zap, Trash2, Search as SearchIcon, Sparkles, Database } from 'lucide-react';
    ...
    <div className="min-h-screen p-6" style={{ background: '#07071a' }}>
      ...
      <AdminOnboardingTour
        ...
        onSkip={() => setShowTour(false)} />
            ...
            variant="outline"
            size="sm"
            onClick={() => setShowTour(true)} className="bg-white text-slate-800 px-3 text-xs font-medium rounded-md inline-flex items-center justify-center gap-2 whitespace-nowrap transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:size-4 [&_svg]:shrink-0 border dark:border-gray-600 dark:bg-gray-800 dark:text-white shadow-sm dark:hover:bg-gray-700 h-8 border-white/20 hover:bg-white/10 hover:text-white">
                ...
                <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
                    ...
                    <TabsContent value="overview" className="space-y-6">
                        <AdminOverviewTab
                            ...
                            analytics={analytics}
                            allReferenceDocs={allReferenceDocs}
                            allUsers={allUsers}
                            allConversations={allConversations}
                            onSelectRequest={(request) => { setSelectedRequest(request); setAdminNotes(''); }}
                            onApproveDoc={(doc) => approveDocMutation.mutate(doc)}
                            onDenyDoc={(doc) => denyDocMutation.mutate(doc)}
                            approveDocPending={approveDocMutation.isPending}
                            denyDocPending={denyDocMutation.isPending}
                        />
                    </TabsContent>
